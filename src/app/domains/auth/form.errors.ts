...
export class Ai1FormValidatorError {
  static getFormControlErrorText(
    ...
    if (ctrl.hasError('required')) {
      ...
      return `${camelCaseToText(name).toUpperCase()} is shorter than ${
        ctrl.errors?.['minlength'].requiredLength
      }`;
    } else if (ctrl.hasError('maxlength')) {
      return `${camelCaseToText(name).toUpperCase()} is longer than ${
        ctrl.errors?.['maxlength'].requiredLength
      }`;
    } else if (ctrl.hasError('email')) {
      return `${camelCaseToText(name).toUpperCase()} is not a valid.`;
    } else if (ctrl.hasError('min')) {
      return `${camelCaseToText(name).toUpperCase()} should be greater than ${
        ctrl.errors?.['min'].min
      }`;
    } else if (ctrl.hasError('max')) {
      return `${camelCaseToText(name).toUpperCase()} should be smaller than ${
        ctrl.errors?.['max'].max
      }`;
    } else {
